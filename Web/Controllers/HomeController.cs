...
{
    public class HomeController : App.Framework.Mvc.BaseController
    {
        ...
        public ActionResult Index()
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                {
                    string UserID = Domain.SiteConfiguration.UserName.ToString();
                   ...
                   if (!String.IsNullOrWhiteSpace(RoleName) && RoleName.Contains("DTS"))
                    {
                        return RedirectToAction("Home", "DTS");
                    }
                    ...
                    {
                        if (UserRole.RoleID == 6) //ReportPartCounter
                            return View("index.int");
                        else if (UserRole.RoleID == 7) //ReportSupply
        ...
        public ActionResult GetListToDoList()
        {
            var data = Service.Master.ToDoLists.GetToDoListPartCounter(User.Identity.GetUserId());
            var list = data.OrderByDescending(o => o.CreatedOn).ToList();
            return Json(list, JsonRequestBehavior.AllowGet);
        }

        public ActionResult GetListToDoListSupply()
        {
            var data = Service.Master.ToDoLists.GetToDoListSupply(User.Identity.GetUserId());
            //var list = data.OrderByDescending.ToList();
