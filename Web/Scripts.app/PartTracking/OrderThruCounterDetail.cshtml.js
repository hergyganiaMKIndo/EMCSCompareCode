...
$(function () {
    ...
    $tableDet.bootstrapTable({
        ...
        //        fixedNumber:3,
        columns: [
            { field: 'action', title: 'Action<br/>  ', width: '185px', align: 'center', formatter: operateFormatter, events: operateEventsDet, switchable: false },
            { field: 'XFORNO', title: 'Mat. Doc No', width: '105px', halign: 'center', align: 'center', sortable: true },
            { field: 'SOS', title: 'Material Type', width: '100px', halign: 'center', align: 'left', sortable: true, switchable: false },
            { field: 'part_no', title: 'Material No', width: '150px', halign: 'center', align: 'left', sortable: true, switchable: false },
            { field: 'part_desc', title: 'Material Desc', width: '230px', halign: 'center', align: 'left', sortable: true, switchable: false },
            ...
            { field: 'seg_no', title: 'Seg', width: '70px', halign: 'center', align: 'center', sortable: true },
		    ...
		    { field: 'weight', title: 'Weight', width: '80px', halign: 'center', align: 'right', sortable: true, formatter: ConvertWeight, footerFormatter: function totWt(data) { return '<span class="totalWeight"></span>'; } },
            ...
            { field: 'shipment_type', title: 'Shipment Type', width: '150px', halign: 'center', align: 'left', sortable: true },
            ...
            { field: 'RLPACD', title: 'Replaced', width: '65px', halign: 'center', align: 'center', sortable: true },
            { field: 'RPPANO', title: 'Replace<br>Part No', width: '95px', halign: 'center', align: 'center', sortable: true },
            { field: 'REMAN', title: 'REMAN', width: '65px', halign: 'center', align: 'center', sortable: true }
            //,
            //{ field: 'CodeName', title: 'TRX CD Desc', width: '200px', halign: 'center', align: 'center', sortable: true }
        ]
    });
    ...
    window.pis.table({
        ...
        searchParams: $('#ref_doc_no_det').val() == "-" ? $('#wo_number_det').val() : $('#ref_doc_no_det').val(),
        autoLoad: true,
...
function operateFormatter(value, row, index) {
    var btn = '';
    if (row.status == 'Supplied Ex-stock' || row.status == 'Un-Submitted Backorder') {
        btn = '<div class="btn-group"> - </div>';
    }
    else {
        btn = '<div class="btn-group">' +
//			'<button type="button" class="btn btn-info detail" title="Detail" onclick="clickDetail(\'' + row.part_no + '\',\'' + row.part_desc + '\')" href="#modal"><i class="fa fa-search-plus"></i> Detail</button>' +
...
function clickDetail(part_no, part_desc) {
    $('#modal').modal({
        backdrop: true,
        keyboard: false,
        show: true,
    });
    $('#d_part_no').val(part_no);
    $('#d_part_desc').val(part_desc);
    //    alert(row.part_no);
}

function ConvertWeight(value, row, index) {
...
function progressFormat(value, row, index) {
    ...
    if (row.status == 'Full GR') {
        seq = 5;
        perc = 100;
    } else if (row.status == 'Un-Submitted Backorder') {
        seq = 1;
        perc = 20;
    }
    else if (row.status == 'Submitted Backorder') {
        seq = 2;
        perc = 50;
    }
    else if (row.status == 'Partial GR') {
        seq = 4;
        perc = 80;
    }
