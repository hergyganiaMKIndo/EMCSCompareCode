...
ExcelPlus.prototype = {
  ...
  */
   ...
   */
  openLocal:function(params, onReady, onUpload) {
        ...
        if (typeof FileReader === "function") {
            ...
            // Event Input
            document.getElementById(params.idButton).outerHTML = '<div id="ep-' + params.idButton + '" class="input-group ep-file"></div>';// _inputGroup[0].outerHTML;
            var _inputGroup = $('#ep-' + params.idButton);
            _inputGroup.append('<input class="form-control ep-file-caption" tabindex="500" onkeydown="return false;" onpaste="return false;" placeholder="' + params.labelButton + '" />');
            _inputGroup.append('<div class="input-group-btn input-group-append">' +
                '<button type="button" tabindex="500" title="Clear all unprocessed files" class="btn btn-default ep-remove-button" > <i class="glyphicon glyphicon-trash"></i> <span class="hidden-xs">Remove</span></button >' +
                '<button type="submit" tabindex="500" title="Upload selected files" class="btn btn-default ep-upload-button"><i class="glyphicon glyphicon-upload"></i><span class="hidden-xs">Upload</span></button>' +
                '<div class="btn btn-primary ep-btn-file">' +
                '<span class="ep-browse hidden-xs">Browse ...</span>' +
                '</div></div>');
            _inputGroup.append('<input id="' + params.idButton + '" type="file" value="' + params.labelButton + '" />');

            var _fileCaption = _inputGroup.find('input.ep-file-caption');
            var _btnRemove = _inputGroup.find('button.ep-remove-button');
            var _btnUpload = _inputGroup.find('button.ep-upload-button');
            var _btnBrowse = _inputGroup.find('div.ep-btn-file > span.ep-browse');
            var _inputFile = _inputGroup.find('input#' + params.idButton);

            _btnRemove.css('display', 'none');
            _btnUpload.css('display', 'none');
            _inputFile.css('display', 'none');

            _btnBrowse.on('click', function (e) {
                _inputFile.click();
            });
            _btnRemove.on('click', function (e) {
                _fileCaption.attr('placeholder', params.labelButton);
                _inputFile.val('');
                _btnRemove.hide();
                _btnUpload.hide();
            });
            _btnUpload.on('click', function (e) {
                onUpload.call(_this, true);
            });
            ...
            addEventListener(document.getElementById(params.idButton), "change", function(e) {
                ...
                fileReader.onload = function (e) {
                    ...
                    try {
                        ...
                        //======================================================================
                        _fileCaption.attr('placeholder', file.name);
                        _btnRemove.show();
                        _btnUpload.show();
                        //======================================================================
                    } catch (err) {
                        //console.log(err.message);
                        onReady.call(_this, false, err);
                    }
